...
use crate::kernels::template::addcmul::{
    io::{
...
>(
    ...
    let key = |(base, diff, scale): &(CubeTensor<R>, CubeTensor<R>, CubeTensor<R>)| {
    ...
    let input_gen =
        |_key: &AddcmulForwardAutotuneKey,
         (base, diff, scale): &(CubeTensor<R>, CubeTensor<R>, CubeTensor<R>)| {
            (base.copy(), diff.copy(), scale.copy())
        };
        ...
        for line_size in LINE_SIZE_CANDIDATES {
            set = set.with(
                Tunable::new(
                    ...
                    move |(base, diff, scale)| {
                        Ok::<_, String>(addcmul::<R, F>(base, diff, scale, line_size))
                    },
                )
...
mod fusion_impl {
    ...
    use super::*;
    use crate::kernels::template::addcmul::AddcmulBackend;
