...
pub trait TemplateBackend: burn::tensor::backend::Backend {
...
pub fn matmul_add_relu_custom<B: TemplateBackend>(inputs: MatmulAddReluInputs<B>) -> Tensor<B, 3> {
    inputs.check().unwrap();
...
pub fn matmul_add_relu_reference<B: TemplateBackend>(
