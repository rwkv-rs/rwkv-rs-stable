...
use crate::kernels::template::matmul_add_relu::{
    MatmulAddReluBackend,
    io::MatmulAddReluPrimitiveInputs,
};

// Implement our custom backend trait for any backend that also implements our custom backend trait.
impl<B, C> MatmulAddReluBackend for Autodiff<B, C>
where
    B: MatmulAddReluBackend,
    C: CheckpointStrategy,
{
    ...
    fn fused_matmul_add_relu(inputs: MatmulAddReluPrimitiveInputs<Self>) -> FloatTensor<Self> {
        ...
        // Please name this structure "Fused{KernelName}Backward" when you reuse this template.
        #[derive(Debug)]
        ...
        impl<B: MatmulAddReluBackend> Backward<B, 3> for FusedMatmulAddReluBackward {
        ...
        match FusedMatmulAddReluBackward
            .prepare::<C>([
                inputs.lhs.node.clone(),
                inputs.rhs.node.clone(),
                inputs.bias.node.clone(),
            ])
            ...
            .stateful()
        {
            OpsKind::Tracked(mut prep) => {
                ...
                let lhs_state = prep.checkpoint(&inputs.lhs);
                let rhs_state = prep.checkpoint(&inputs.rhs);
                let bias_shape = inputs.bias.primitive.shape();

                let output = B::fused_matmul_add_relu(MatmulAddReluPrimitiveInputs {
                    lhs: inputs.lhs.primitive.clone(),
                    rhs: inputs.rhs.primitive.clone(),
                    bias: inputs.bias.primitive,
                });
            ...
            OpsKind::UnTracked(prep) => {
                ...
                // keeping any state.
                let output = B::fused_matmul_add_relu(MatmulAddReluPrimitiveInputs {
                    lhs: inputs.lhs.primitive,
                    rhs: inputs.rhs.primitive,
                    bias: inputs.bias.primitive,
                });
                prep.finish(output)
            }
