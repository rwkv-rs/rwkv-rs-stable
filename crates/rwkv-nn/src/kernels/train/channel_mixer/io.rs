...
pub struct ChannelMixerForwardInputs<B: burn::tensor::backend::Backend> {
    ...
    /// Key projection weight shaped `[embedded_dim, expanded_dim]`.
    pub key_weight: Tensor<B, 2>,
    /// Value projection weight shaped `[expanded_dim, embedded_dim]`.
    ...
    B: Backend,
{
    pub(crate) fn check(&self) -> Result<(), KernelInputsError<B>> {
        ...
        let expanded_dim = key_weight.dim(1);

        check_axis_non_empty(embedded_context.axis(2))?;
        check_axis_non_empty(key_weight.axis(1))?;
        ...
        check_axes_equal(&[key_weight.axis(1), value_weight.axis(0)])?;
        check_shape(&value_weight, [expanded_dim, embedded_dim])?;
...
pub struct ChannelMixerForwardPrimitiveInputs<B: Backend> {
    ...
    /// Primitive key projection weight shaped `[embedded_dim, expanded_dim]`.
    pub key_weight: FloatTensor<B>,
    /// Primitive value projection weight shaped `[expanded_dim, embedded_dim]`.
...
pub struct ChannelMixerBackwardPrimitiveOutputs<B: Backend> {
    ...
    /// Gradient for `key_weight`, shaped `[embedded_dim, expanded_dim]`.
    pub key_weight_grad: FloatTensor<B>,
    /// Gradient for `value_weight`, shaped `[expanded_dim, embedded_dim]`.
