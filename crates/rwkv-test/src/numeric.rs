/// Numeric tolerances for floating-point tensor comparison.
...
/// Numeric statistics used by tensor comparison.
#[derive(Clone, Copy, Debug)]
...
/// Computes absolute error, relative error, and cosine similarity statistics.
pub fn numeric_stats(actual: &[f64], expected: &[f64], eps: f64) -> NumericStats {
    assert_eq!(
        actual.len(),
        expected.len(),
        "numeric comparison length mismatch: actual={} expected={}",
        actual.len(),
        expected.len()
    );
...
/// Asserts that two decoded floating-point tensors satisfy the supplied tolerance.
pub fn assert_values_close(
    ...
    let stats = numeric_stats(actual, expected, tolerance.rel_eps);
    assert!(
